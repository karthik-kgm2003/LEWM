...
Hierarchical Bayesian Model (HBM) Formulation

(Note: The following equations are presented for conceptual clarity. A full implementation would involve defining appropriate prior distributions for all parameters.)

Level 1: The Within-Person (Daily) Model

At Level 1, we model the daily well-being W (e.g., a composite score of positive affect and engagement) for person i on day t.

$$W_{it} = \beta_{0i} + \beta_{1i}(\text{DailyDemands}_{it}) + \beta_{2i}(\text{DailyResources}_{it}) + \epsilon_{it}$$

Where:

$W_{it}$: The well-being score for person i on day t.

$\text{DailyDemands}_{it}$: EMA-reported workload, negative events (AET).

$\text{DailyResources}_{it}$: EMA-reported peer support, clarity (AET).

$\beta_{0i}$: The individual's personal baseline wellbeing.

$\beta_{1i}$: The individual's personal sensitivity to daily demands.

$\epsilon_{it}$: Daily error term, $\epsilon_{it} \sim N(0, \sigma^2)$.

Level 2: The Between-Person (Stable) Model

At Level 2, we model the Level 1 coefficients ($\beta_{0i}$ and $\beta_{1i}$) as outcomes.

Modeling the Baseline ($\beta_{0i}$):

$$\beta_{0i} = \gamma_{00} + \gamma_{01}(\text{Autonomy}_{i}) + \gamma_{02}(\text{Relatedness}_{i}) + \gamma_{03}(\text{Context}_{j}) + u_{0i}$$

$\gamma_{00}$: The grand mean of wellbeing for the entire company.

$\text{Autonomy}_{i}$: The person's stable, survey-measured perceived autonomy (SDT).

$\text{Context}_{j}$: An objective contextual variable for their team j (from Layer 1b), e.g., $\text{MeetingLoad}_{j}$.

$u_{0i}$: The individual's random deviation from the team mean, $u_{0i} \sim N(0, \tau_{0}^2)$.

Modeling the Interaction (The "Cross-Level" Effect):

$$\beta_{1i} = \gamma_{10} + \gamma_{11}(\text{Autonomy}_{i}) + u_{1i}$$

$\gamma_{10}$: The average effect of demands on wellbeing.

$\gamma_{11}$: The key interaction term. This coefficient tests the hypothesis that Autonomy (SDT) buffers the negative impact of DailyDemands (JD-R). We would hypothesize $\gamma_{11}$ to be positive (a high-autonomy person has a less negative $\beta_{1i}$).

$u_{1i}$: The individual's random deviation, $u_{1i} \sim N(0, \tau_{1}^2)$.

Level 3: The Predictive Outcome Model (Attrition)

The outputs from the HBM (e.g., an individual's baseline wellbeing $\beta_{0i}$ and, most importantly, their trend or slope over time) are then fed as features into a predictive model for a binary outcome, like voluntary turnover.

$$P(\text{Attrition}_i) = \text{logit}^{-1}(\theta_0 + \theta_1(\beta_{0i}) + \theta_2(\text{Trend}_{W,i}) + \theta_3(\text{Variability}_{\text{Affect},i}) + \theta_4(\text{Context}_j))$$
